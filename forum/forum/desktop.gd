...
func _show_message_popup(title: String, message: String, show_cancel: bool = false, on_confirm: Callable = func(): pass):
	...
	var popup = Panel.new()
	popup.size = Vector2(340, dynamic_height)  # 增加宽度和动态高度
	...
	# 内容 - 关键修复：设置正确的宽度和自动换行
	var msg_label = Label.new()
	msg_label.text = message
	msg_label.add_theme_font_size_override("font_size", 14)
	msg_label.autowrap_mode = TextServer.AUTOWRAP_WORD  # 自动换行
	...
	confirm_btn.text = "确定"
	confirm_btn.custom_minimum_size = Vector2(80, 30)  # 设置按钮固定大小
